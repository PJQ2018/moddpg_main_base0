...
WIDTH = 10  # 地图的宽度
HEIGHT = 10  # 地图的高度
UNIT = 100  # 每个方块的大小（像素值）   1000m*1000m的地图
# C = 5000  # 传感器的容量都假设为5000
P_u = pow(10, -5)  # 传感器的发射功率 0.01mW
P_d = 15  # 无人机下行发射功率 15W
H = 20.  # 无人机固定悬停高度 20m
B = pow(10, 6) #带宽 1M
R_d = 20.  # 无人机充电覆盖范围 20m
N_S_ = 100  # 设备个数
V = 35  # 无人机最大速度 35m/s
...
class UAV(tk.Tk, object):
    def __init__(self, R_dc=20.):
        ...
        self.N_UAV = 1 #无人机个数
        self.max_speed = V  # 无人机最大速度 35m/s
        self.H = 20.  # 无人机飞行高度 20m
        self.X_min = 0 # 地图边界
        self.Y_min = 0
        self.X_max = (WIDTH) * UNIT
        self.Y_max = (HEIGHT) * UNIT  # 地图边界
        self.R_dc = R_dc  # 水平覆盖距离 20m
        self.sdc = math.sqrt(pow(self.R_dc, 2) + pow(self.H, 2))  # 最大服务长度
        self.noise = pow(10, -12)  # 噪声功率为-90dbm
        self.AutoUAV = [] #无人机组编号，这里没有用
        self.Aim = []
        self.Pass = [] #已经服务的用户数
        self.N_AIM = 1  # 选择服务的用户数
        self.FX = 0. #撞墙次数

        self.data = load_paths()  #加载 best_paths6.json 文件中的数据
        self.paths = self.data["best_paths"]  # 所有回合路径（列表的列表）
        self.now_id = 1  # 用于选择目标设备
        self.SoPcenter = np.array(self.data["SoPcenter"])  # 提取设备位置

        #   以 v in [0,1], 角度 in [-1,1]为动作
        self.action_space = spaces.Box(low=np.array([0., -1.]), high=np.array([1., 1.]),
                                       dtype=np.float32)
        self.state_dim = 5
        # 状态空间为 目标用户位置与无人机的相对位置，无人机位置，是否撞墙
        self.state = np.zeros(self.state_dim)
        self.xy = np.array([[0, 0]])  # 初始化无人机初始位置
        self.xy0 = np.array([[0, 0]])  # 初始化基站位置
        self.b_S = np.array(self.data["b_S"])  # 提取数据量
        self.round = -1
        self.remaining_time = 600  # 剩余时间
    ...
    def reset(self):
        self.render()
        # self.device_selection_count[self.all_id] += 1
        ...
        self.round += 1
        self.target = self.paths[self.round % 9]
        ...
        self.state = np.concatenate(((self.pxy - self.xy[0]).flatten() / 1000., self.xy.flatten() / 1000., [0.]))
    ...
    def step_move(self, action, above=False):
        ...
        dis = np.linalg.norm(self.xy - self.xy0)
        v = 10
        self.t_need = dis / v #现位置到原点需要的时间
        ...
        if (above == False and self.dis[self.idx_target] <= self.sdc) or (
            ...
            if self.remaining_time - self.t_need > 0:
                EcH = Ech * T_hover1
                ...
                # self.device_selection_count[self.idx_target] += 1
            else:
        ...
        return state_, reward, Done, Dr, EcE, EcF, EcH, T_hover, idu, self.remaining_time - self.t_need  # 状态值，奖励，是否到达目标，总数据率, 覆盖到的用户，收集能量，无人机能耗
