...
class AGENT(object):
    ...
    def __init__(self, args, a_num, a_dim, s_dim, a_bound, is_train):
        ...
        w_init = tf.initializers.he_normal(1)
    ...
    def learn(self):
        ...
        br = bt[:, self.s_dim + self.a_dim:self.s_dim + self.a_dim + 1]  # 从bt获得数据r
        bs_ = bt[:, self.s_dim + self.a_dim + 1:]  # 从bt获得数据s_
        ...
        with tf.GradientTape() as tape:
            a = self.actor(bs)
            q = self.critic([bs, a])
            a_loss = -tf.reduce_mean(q)  # 梯度上升，最大化Actor网络的Q值。
    ...
    def load_ckpt(self, version, eps):
        ...
        :return: None
        """
        model_path = os.getcwd() + '/{}/{}/'.format(version, 'models1')
